        ...
        <div className={TI_CLASSES.leftContainer}>
           {showCompleteCheckbox && (
            ...
            className={`${TI_CLASSES.completeBtn} ${isCompleted ? "text-green-500" : "text-gray-300"}`}>
             <CheckCircle2 size={13} className={`${TI_CLASSES.checkboxIconBase}
             ${isCompleted ? "fill-green-500" : ""}`}/>
            </button>
           )}

           <div className='flex-1 min-w-0'>
             <div className='flex items-baseline gap-2 mb- flex-wrap'>
        ...
        <div className={TI_CLASSES.rightContainer}>
          <div className='relative'>
             <button onClick={() => setShowMenu(!showMenu)}
                className={TI_CLASSES.menuButton}>
                 <MoreVertical className='w-4 h-4 cm:w-5 sm:h-5' size={8}/>
             </button>
          ...
          <div>
            ...
            <div className={TI_CLASSES.createdRow}>
               ...
               {task.createdAt ? 
               `Created ${format(new Date(task.createdAt),"MMM dd")}`:"No data"}
            </div>
          </div>
        </div>
     </div>
